...
**Always look up**: library API details, framework feature lists, version-specific behavior, current best practice, recent changes — anything where being wrong costs time. Particularly relevant here for semgrep rule syntax, ESLint plugin APIs, and tsup/vitest config shapes.
...
5. **Evidence-check the diff.** If the diff cites library behavior, version-specific features, or external API shapes you cannot fully verify from the code alone, query exa to confirm. Semgrep rule syntax and ESLint plugin shapes change between versions — verify before approving.
6. **Dependency scrutiny.** If the diff adds a new top-level dependency (Python or JS), confirm the PR description includes the new-dependency proposal (research, alternatives, maintainer signals). Missing proposal = blocker. Spot-check the proposal's claims via exa. Confirm no existing project dependency could have solved the problem, and that JS additions respect the peer-dependency preference for ESLint-ecosystem packages.
