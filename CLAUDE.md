...
**Always query MCPs for**: library API details, framework feature lists, version-specific behavior, current best practice, recent changes — anything where being wrong costs time. Particularly relevant here for semgrep rule syntax, ESLint plugin APIs, and tsup/vitest config shapes.
...
- **context7** (`mcp__context7__resolve-library-id`, `mcp__context7__query-docs`) — authoritative current docs for a specific library / framework / SDK / CLI. Use whenever you need to know how to call something correctly (semgrep, ESLint flat config, tsup, vitest, etc.).
...
Remember the library-only constraint already documented above: CLI dependencies do not belong here.

## New dependency proposal (BLOCKING)

If your work requires adding a new top-level dependency (Python `python/pyproject.toml` or JS `js/package.json`), STOP before installing it.
...
3. **Wait for approval.** Install only after explicit user approval (`uv add` for Python, `pnpm add` for JS). If rejected, revisit the solution preference order.

The bar is especially high in this repo: pragma-lint is consumed by every downstream Pragmatiks package, and JS deps reach every consumer's ESLint config. For JS, prefer peer dependencies over hard runtime deps for anything in the ESLint ecosystem (as already noted in the Dependency Policy).
...
5. **Evidence-check the diff.** If the diff cites library behavior, version-specific features, or external API shapes you cannot fully verify from the code alone, query context7 / deepwiki / exa to confirm. Semgrep rule syntax and ESLint plugin shapes change between versions — verify before approving.
6. **Dependency scrutiny.** If the diff adds a new top-level dependency (Python or JS), confirm the PR description includes the new-dependency proposal (research, alternatives, maintainer signals). Missing proposal = blocker. Spot-check the proposal's claims via exa or deepwiki. Confirm no existing project dependency could have solved the problem, and that JS additions respect the peer-dependency preference for ESLint-ecosystem packages.
